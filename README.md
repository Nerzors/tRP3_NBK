<div align="center">
<img src="https://media.nerzors.de/addons/nbk/nbkAddon_tRP3.final-v1.png" width="128" alt="Nerzors Blacklist Keeper Logo with Total RP3 Tooltip"/>

<h1>Total RP 3: Nerzors Blacklist Keeper Tooltip Support</h1>

<p><i>Remember the players you never want to group with again... and the good ones too.</i></p>

<p>
  <a href="https://www.curseforge.com/wow/addons/trp3-nbk-tooltip">
    <img src="https://img.shields.io/badge/CurseForge-1B1F2B?style=for-the-badge&logo=curseforge&logoColor=e04e14&labelColor=1B1F2B" alt="CurseForge"/>
  </a>
  <a href="https://www.curseforge.com/wow/addons/trp3-nbk-tooltip">
    <img src="https://img.shields.io/badge/WoWUp-1B1F2B?style=for-the-badge&logoColor=A87CE8&labelColor=1B1F2B" alt="WoWUp"/>
  </a>
  <a href="https://addons.wago.io/addons/trp3-nbk-tooltip">
    <img src="https://img.shields.io/badge/Wago.io-1B1F2B?style=for-the-badge&logoColor=A87CE8&labelColor=1B1F2B" alt="Wago.io"/>
  </a>
</p>

<p>
  <img src="https://cf.way2muchnoise.eu/1554935.svg?gameVersionTypeId=517" alt="CurseForge Downloads"/>
  <img src="https://img.shields.io/github/v/release/Nerzors/tRP3_NBK?style=flat-square&color=252669&labelColor=1B1F2B" alt="Latest Release"/>
  <img src="https://img.shields.io/github/last-commit/Nerzors/tRP3_NBK?style=flat-square&color=A87CE8&labelColor=1B1F2B" alt="Last Commit"/>
  <img src="https://img.shields.io/github/issues/Nerzors/tRP3_NBK?style=flat-square&color=252669&labelColor=1B1F2B" alt="Open Issues"/>
  <img src="https://img.shields.io/github/stars/Nerzors/tRP3_NBK?style=flat-square&color=A87CE8&labelColor=1B1F2B" alt="Stars"/>
</p>

</div>

<br/>

This module ist for Total RP 3 enables NerzorsBlacklistKeeper's tooltip functionality to work with TRP3's custom tooltip.

This addon requires both Total RP 3 and NerzorsBlacklistKeeper to be installed.

## ADDON - Core Information

| Property | Value |
|----------|-------|
| Author | Veplo |
| Co-Author | Nerzors |
| Language | Multi-Language |
| Game Version | WoW Retail, Classic Era and Classic MoP |
| Current Branch | 1.0.0 |

## Who find NerzorsBlacklistKeeper?
On CurseForge:
- For Retail [12.0.7]
  - All-In-One Version (Currently only for Retail): [https://www.curseforge.com/wow/addons/nerzorsblacklistkeeper](https://www.curseforge.com/wow/addons/nerzorsblacklistkeeper)
- For Retail [12.0.7], Classic Era [1.15.8] and Classic MoP [5.5.4]
  - Core Only Version [https://www.curseforge.com/wow/addons/nerzorsblacklistkeepercore](https://www.curseforge.com/wow/addons/nerzorsblacklistkeepercore)

On WagoIO:
- For Retail [12.0.7]
  - Core Only Version: [https://addons.wago.io/addons/nerzorsblacklistkeeper-core-only](https://addons.wago.io/addons/nerzorsblacklistkeeper-core-only)
  - All-In-One Version (Currently only for Retail): [https://addons.wago.io/addons/nerzorsblacklistkeeper](https://addons.wago.io/addons/nerzorsblacklistkeeper)
- For Classic Era [1.15.8] and Classic MoP [5.5.4]
  - Classic Core Only Version [https://addons.wago.io/addons/nerzorsblacklistkeeper-classic](https://addons.wago.io/addons/nerzorsblacklistkeeper-classic)

On Github:
- https://github.com/Nerzors/NerzorsBlacklistKeeper/